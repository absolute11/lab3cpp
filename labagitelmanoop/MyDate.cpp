...
int MyDate::daytab[2][13] = { {0,31,28,31,30,31,30,31,31,30,31,30,31},{0,31,29,31,30,31,30,31,31,30,31,30,
31} 
};
...
int MyDate::equal(const MyDate& t) const {
	return (day == t.day) && (month == t.month) && (year == t.year) ? 1 : 0;
}
...
char* MyDate::get_DayOfWeek() {
	...
	size_t size = strlen(DayNames[dd]) + 1;
	char* s = new char[size];
	strcpy_s(s, size, DayNames[dd]);
	return s;
}
